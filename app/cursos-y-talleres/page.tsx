...
export default function CulturalAgendaPage() {
	...
	const [checkboxesDependencia, setCheckboxesDependencia] = useState<CheckboxItem[]>([
				...
				<div className="px-4 lg:px-[104px] bg-white pb-14">
					<div className="container">
						<div className="max-w-[814px] mx-auto">
							<Title className="text-center">Cursos y talleres</Title>
							<div className="leading-[24px] text-dark-blue-2">
								<p>En el Centro Cultural de San Marcos te ofrecemos cursos y talleres para niños, jóvenes y adultos. Tienes a tu disposición una variada lista de opciones: talleres musicales, de danza, artes plásticas, cursos especializados y mucho más. Te invitamos a revisar a continuación las opciones de cada una de las diez direcciones que conforman el Centro Cultural de la Universidad Decana de América.</p>
							</div>
							<div className="mt-20">
								<Search className="max-w-[422px] mx-auto" placeholder="¿Qué te gustaría aprender?" onSearch={handleSearch} />
							</div>
						</div>
					</div>
				</div>
						...
						<div className="flex flex-row justify-between gap-x-[105px]">
								...
								<div className="bg-white rounded-2xl p-6 ring-1 ring-inset ring-dark-white-3 w-[314px] text-dark-blue-2">
									<div>
										<div className="flex items-center justify-between h-6 mb-2">
											<span className="font-bold leading-[19.2px]">Tipo</span>
											<ArrowDropdownIcon />
										</div>
										<div className="flex flex-col gap-y-1">
											{checkboxesTipo.map((checkbox) => (
												<Checkbox
													...
													onChange={(e) => handleCheckboxChangeTipo(checkbox.id, e.target.checked)}
												/>
											))}
										</div>
									</div>
									...
									<div>
										<div className="flex items-center justify-between h-6 mb-2">
											<span className="font-bold leading-[19.2px]">Dependencia</span>
											<ArrowDropdownIcon />
										</div>
										<div>
											<div className="flex flex-col gap-y-1">
												{checkboxesDependencia.map((checkbox) => (
													<Checkbox
														key={checkbox.id}
														label={checkbox.label}
														checked={checkbox.checked}
														onChange={(e) => handleCheckboxChangeDependencia(checkbox.id, e.target.checked)}
													/>
												))}
											</div>
										</div>
							...
							<div>
								...
								<ul className="flex flex-col space-y-8">
									{
										coursesAndWorkshopsDataToHome.map((event, index) => (
											<li className="flex" key={index}>
												<CoursesAndWorkshopsCard {...event} />
											</li>
										))
									}
								</ul>
								<Pagination
									...
									totalCount={coursesAndWorkshopsDataToHome.length}
									pageSize={PageSize}
